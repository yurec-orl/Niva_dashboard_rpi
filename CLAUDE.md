...
- `UIEvent::Restart` (`page_framework/page_manager.rs`) is a no-op stub — implement or remove.
...
- `graphics/default_style.json` is dead: never loaded (only a commented-out call in `main.rs`), and its keys (`needle_color`, `gauge_minor_mark_count: 4`, etc.) don't match the constants actually used in `ui_style.rs` (`GAUGE_NEEDLE_COLOR`, `GAUGE_MINOR_MARK_COUNT: 37`, ...). Delete it or reconcile it with the real style schema.
