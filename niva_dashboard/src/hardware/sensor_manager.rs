...
impl SensorManager {
    ...
    /// Inserts a value computed outside any registered chain -- e.g. HeadingFusionSensor,
    /// which reads GnssFrame/Bno085Frame directly rather than through a HWAnalogProvider
    /// (see heading_fusion_sensor.rs for why) -- into the same map every chain-backed
    /// reading lands in, so get_sensor_value/get_sensor_values don't need to know how a
    /// given HWInput's value was produced.
    pub fn set_external_value(&mut self, input: HWInput, value: SensorValue) {
...
mod tests {
    ...
    fn test_read_all_sensors_one_failing_chain_does_not_block_others() {
        ...
        let failing_chain = SensorAnalogInputChain::new(
            Box::new(AlwaysFailingAnalogDataProvider { input: HWInput::HwGnssMovingHeading }),
            vec![],
        ...
        assert!(manager.get_sensor_value(&HWInput::HwGnssMovingHeading).is_none(),
